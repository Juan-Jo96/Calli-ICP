...
shared ({caller = owner}) actor class IndexCanister() = this {
  ...
  public shared query({caller = _caller}) func getCanistersByPK(pk: Text): async [Text] {
  ...
  func createWaitlistDBCanister(pk: Text, controllers: ?[Principal]): async Text {
    ...
    let newWaitlistDBCanister = await WaitlistDB.WaitlistDB({
      _owners = ?[owner, Principal.fromActor(this)];
      partitionKey = pk;
      scalingOptions = {
  ...
  public shared({caller = caller}) func deleteLoggedInUser(): async () {
    ...
    if (canisterIds == []) {
      ...
      let _statusMap = await Admin.transferCyclesStopAndDeleteCanisters(canisterIds);
